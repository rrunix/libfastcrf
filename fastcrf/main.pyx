from .extraction_problem cimport make_problem, create_extraction_state, ExtractionProblem, ExtractionContext, Solution, SplitPoint
from .extractor cimport extract_counterfactual_impl
from .converter import convert_rf_format, export
from .observations cimport distance
...
cdef cpppair[int, double] get_closest_counterfactual(parsed_rf, X, sample, dataset_info, ExtractionProblem & problem):
    ...
    cdef cppvector[double] sample_c = adapt_observation_representation(sample, dataset_info)

    cdef int best_idx = counterfactuals[0]
    cdef cppvector[double] counterfactual_c = adapt_observation_representation(X[best_idx], dataset_info)
    cdef double best_distance = distance(problem, sample_c, counterfactual_c)
    ...
    return cpppair[int, double](best_idx, best_distance)
...
def batch_extraction(sklearn_rf, dataset_info, X, max_distance=-1,
    ...
    cdef ExtractionProblem problem = make_problem(parsed_rf, dataset_info, search_closest, log_every,
                                                  max_iterations)
...
cdef extract_single_counterfactual_set(ExtractionProblem & problem, sklearn_rf, parsed_rf, dataset_info, to_explain, dataset):
    cdef cpppair[int, double] closest_mo
    ...
    cdef ExtractionContext global_state = create_extraction_state(problem, parsed_rf, factual_class, to_explain,
                                                                  dataset_info, closest_mo.second)
    ...
    if solution.found:
        ...
        res['explanation'] = counterfactual_set_from_obs(parsed_rf, dataset[closest_mo.first], dataset_info, (factual_class + 1) % 2)
        res['explanation_compact'] = res['explanation']
        res['explanation_compact_count'] = counterfactual_set_from_obs(parsed_rf,
                                                         dataset[closest_mo.first],
                                                         dataset_info, (factual_class + 1) % 2,
                                                         filter_label_type='count')
        res['feature_bounds_splits'] = res['explanation_compact'].feat_conditions
        res['remaining_rules'] = res['explanation'].rule_ids
        res['set_rules'] = res['explanation'].rule_ids
        res['foil_estimated_class'] = sklearn_rf.predict([dataset[closest_mo.first]])[0]
        res['distance'] = closest_mo.second
        res['found'] = True
    return res
    ...
    for rule_id in rule_ids:
        ...
        if set_trees[t_id] > 1:
            if obs is None or split_point_info is None:
                raise ValueError("Obs and split_point_info should be provided when there is more than one observation in a tree available")
