...
def fit(df, class_name, columns, features_type, discrete, continuous,
    ...
    cmd = "research_paper/methods/LORE/yadt/dTcmd -fd %s -fm %s -sep %s -d %s" % (
    ...
    while not node_isleaf[node]:
        ...
        for child in dt.neighbors(node):
            ...
            if att in discrete:
                ...
                if str(yadt_value2type(edge_val, att, features_type)) == str(val):
...
def predict_rule(dt, x, class_name, features_type, discrete, continuous, log=False):
    edge_labels = get_edge_labels(dt)
    node_labels = get_node_labels(dt)
    node_isleaf = {k: v == 'ellipse' for k, v in nx.get_node_attributes(dt, 'shape').items()}

    #if log:
    #    print(edge_labels, node_labels, node_isleaf, x, class_name)
...
def get_counterfactuals(dt, tree_path, rule, diff_outcome, class_name, continuous, features_type):
    ...
    for node in sp_from_root:
        ...
        # [node_labels[k] for k, v in node_isleaf.items() if v]
        ...
        if str(sp_outcome) == str(int(diff_outcome)):
...
def apply_counterfactual(x, delta, continuous, discrete, features_type, integer_based=[]):
    ...
    for att, val in delta.items():
        ...
        if att in continuous:
            if '>' in val:
                gt = float(yadt_value2type(val.replace('>', ''), att, features_type))
            elif val.count('<') == 2:
                val = val.replace('=', '')
                gt, _, lt  = val.split('<')
                gt = float(gt.strip())
                ...
                lt = float(yadt_value2type(val.replace('<=', ''), att, features_type))

            if gt is not None and gt >= xcf[att]:
                if att in integer_based:
                    xcf[att] = math.floor(gt) + 1
                else:
                    xcf[att] = gt + 0.001

            if lt is not None and lt < xcf[att]:
                if att in integer_based:
                    xcf[att] = math.floor(lt)
                else:
                    xcf[att] = lt

        elif att in discrete:
            xcf[att] = int(yadt_value2type(val, att, features_type))
        else:
