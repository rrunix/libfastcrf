...
def fitness_sso(x0, bb, alpha1, alpha2, eta, discrete, continuous, class_name, idx_features, distance_function, x1):
    ...
    if np.isnan(evaluation):
        print("NAAANfitness_sso")
        print(x0d, x1d, discrete, continuous, class_name)
        print('y0', y0, 'y1', y1, 'sim_ratio', sim_ratio, 'record_similarity', record_similarity, 'alphas', (alpha1, alpha2))
        exit(0)
    return evaluation,
...
def get_oversample(population, halloffame):
    ...
    # print(np.amax(fitness_diff))
    # print(fitness_diff == np.amax(fitness_diff))
