...
class DefragModel(RuleModel):
    def __init__(self, modeltype='regression', maxitr=100, qitr=5, tol=1e-6, eps=1e-10, delta=1e-8, kappa=1e-6, seed=0,
                 restart=10, L=-1, verbose=0, njobs=1):
        ...
        if self.njobs_ == 1:
            for itr in range(self.restart_):
                if self.L_ > 0 and splitter.shape[0] > self.L_:
                    ...
                    s = splitter
                defr = self.fit_defragger(X, y, s, K, fittype, self.modeltype_, self.maxitr_, self.qitr_, self.tol_,
                                          self.eps_, self.delta_, self.seed_ + itr, self.verbose_)
            ...
            for itr in range(self.restart_):
                if self.L_ > 0 and splitter.shape[0] > self.L_:
                    ...
                    s = splitter
                args.append((self.fit_defragger, X, y, s, K, fittype, self.modeltype_, self.maxitr_, self.qitr_,
                             self.tol_, self.eps_, self.delta_, self.seed_ + itr, self.verbose_))
    ...
    def fit_defragger(self, X, y, splitter, K, fittype, modeltype, maxitr, qitr, tol, eps, delta, seed, verbose):
        defragger = Defragger(modeltype=modeltype, maxitr=maxitr, qitr=qitr, tol=tol, eps=eps, delta=delta, seed=seed,
                              verbose=verbose)
...
class Defragger(object):
    def __init__(self, modeltype='regression', maxitr=100, qitr=5, tol=1e-6, eps=1e-10, delta=1e-8, w=1, seed=0,
                 verbose=0):
        self.modeltype_ = modeltype
        self.maxitr_ = maxitr
        self.qitr_ = qitr
    ...
    def __fitFAB(self, X, y, splitter, Kmax, seed):
        ...
        for itr in range(self.maxitr_):
            Qnew = Q.copy()
            if itr < self.qitr_:
                Qnew[:, K] = self.__updateQFAB(y, R, Q[:, K], h[:, K], E[:, K], A[K], eps=self.eps_, w=self.w_,
                                               modeltype=self.modeltype_, maxitr=2)
            else:
                Qnew[:, K] = self.__updateQFAB(y, R, Q[:, K], h[:, K], E[:, K], A[K], eps=self.eps_, w=self.w_,
                                               modeltype=self.modeltype_, maxitr=10)
            ...
            fnew = self.__objFAB(y, R, Qnew, hnew, Enew, Anew, Knew, eps=self.eps_, w=self.w_,
                                 modeltype=self.modeltype_)
            if self.verbose_ > 0:
                if np.mod(itr, self.verbose_) == 0:
