...
def genExp(model_trained, factual_sample, class_labels, aim_label, epsilon, norm_type, dataset_obj, standard_deviations, perform_while_plausibility):
    ...
    factual_class = model_trained.predict([x])[0]
    factual_sample['y'] = True if factual_class == 1 else False
    ...
    aim_label_bool = True if aim_label == 1 else False
    closest_counterfactual_sample['y'] = aim_label_bool
        ...
        if estimator.predict(x.reshape(1, -1) != aim_label):
            ...
            for key in paths_info:
                ...
                if perform_while_plausibility:
                    ...
                    for idx, elem in enumerate(es_instance):
                        ...
                        if attr_obj.attr_type != 'numeric-real':
                            ...
                            es_instance[idx] = max(int(attr_obj.lower_bound),
                                                   min(round(es_instance[idx])), int(attr_obj.upper_bound))
                            # es_instance[idx] = min(
                ...
                if estimator.predict(es_instance.reshape(1, -1)) == aim_label:
                    ...
                    counterfactual_sample['y'] = aim_label_bool
