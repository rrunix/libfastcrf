    ...
    def recurse(node):
        if tree_.feature[node] != _tree.TREE_UNDEFINED:
            ...
            if return_value == 'class_idx_max':
                ...
                prob_array = list(np.divide(tree_.value[node][0], np.sum(tree_.value[node][0])))
                return And(
                    EqualsOrIff(model_symbols['aux']['p0{tree_idx}'.format(tree_idx=tree_idx)]['symbol'], Real(float(prob_array[0]))),
                    EqualsOrIff(model_symbols['aux']['p1{tree_idx}'.format(tree_idx=tree_idx)]['symbol'], Real(float(prob_array[1])))
                )
...
def forest2formula(forest, model_symbols):
    model_symbols['aux'] = {}
    for tree_idx in range(len(forest.estimators_)):
        model_symbols['aux']['p0{tree_idx}'.format(tree_idx=tree_idx)] = {'symbol': Symbol('p0{tree_idx}'.format(tree_idx=tree_idx), REAL)}
        model_symbols['aux']['p1{tree_idx}'.format(tree_idx=tree_idx)] = {'symbol': Symbol('p1{tree_idx}'.format(tree_idx=tree_idx), REAL)}
    ...
    output_formula = Ite(
        GE(
            Plus([model_symbols['aux']['p0{tree_idx}'.format(tree_idx=tree_idx)]['symbol'] for tree_idx in range(len(forest.estimators_))]),
            Plus([model_symbols['aux']['p1{tree_idx}'.format(tree_idx=tree_idx)]['symbol'] for tree_idx in range(len(forest.estimators_))])
        ),
...
def mlp2formula(model, model_symbols):
    ...
    final_layer_binary_feature_string = 'f_{c}_0_post_nonlin'.format(c=len(layer_widths) - 1)
    output_formula = Ite(
        GT(
