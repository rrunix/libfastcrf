...
def getDistanceBetweenSamples(sample_1, sample_2, norm_type, dataset_obj):
  ...
  assert(set(sample_1.keys()) == set(sample_2.keys()))
  ...
  for attr_name_kurz in np.intersect1d(actionable_attributes, one_hot_attributes):
    if attr_name_kurz not in already_considered:
      ...
      if 'cat' in dataset_obj.attributes_kurz[attr_name_kurz].attr_type:
        ...
        raise Exception('{attr_name_kurz} must include either `cat` or `ord`.'.format(attr_name_kurz=attr_name_kurz))
  ...
  if norm_type == 'zero_norm':
    ...
    raise Exception('{norm_type} not recognized as a valid `norm_type`.'.format(norm_type=norm_type))
