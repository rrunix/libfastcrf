...
def runExperiments(dataset_values, model_class_values, norm_values, approaches_values, batch_number, neg_sample_count, target=None, threshold=None):
      ...
      for norm_type_string in norm_values:
        ...
        for approach_string in approaches_values:
          ...
          for factual_sample_index, factual_sample in neg_pred_data_dict.items():
            ...
            counterf = explanation_object['counterfactual_sample']
            best_counterfactual_np = np.array([counterf[col] for col in input_cols])
            explanation_object['valid_'] = model_trained.predict(np.array(best_counterfactual_np).reshape(1, -1)).reshape(-1)[0] == 1
            explanation_object['proba'] = list(model_trained.predict_proba(np.array(best_counterfactual_np).reshape(1, -1)).reshape(-1))
            print(
              f'\tcf_found: {explanation_object["counterfactual_found"]}'
              f'\tcf_valid_: {explanation_object["valid_"]}'
...
if __name__ == '__main__':
  ...
  runExperiments(
    ...
    ['forest'],
    ['one_norm'],
    # ['MO', 'MACE_eps_1e-9', 'FT', 'MACE_eps_1e-9'],
    ['MACE_eps_1e-9'],
    0,
    100,
    target=1, threshold=0.6
  )
