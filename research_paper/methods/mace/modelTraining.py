...
def trainAndSaveModels(experiment_folder_name, model_class, X_train, X_test, y_train, y_test, feature_names):
    ...
    log_file = open('{experiment_folder_name}/log_training.txt'.format(experiment_folder_name=experiment_folder_name),'w')
    ...
    if model_class == 'tree':
        ...
        for tree_idx in range(len(model_trained.estimators_)):
            if SIMPLIFY_TREES:
                print('[INFO] Simplifying decision tree (#{tree_idx + 1}/{len(model_trained.estimators_)})...', end = '', file=log_file)
                model_trained.estimators_[tree_idx].tree_ = treeUtils.simplifyDecisionTree(model_trained.estimators_[tree_idx], False)
                print('\tdone.', file=log_file)
            treeUtils.saveTreeVisualization(model_trained.estimators_[tree_idx], model_class, 'tree{tree_idx}'.format(tree_idx=tree_idx), X_test, feature_names, experiment_folder_name)
    elif model_class == 'lr':
        tmp = 1
    ...
    pickle.dump(model_trained, open('{experiment_folder_name}/_model_trained'.format(experiment_folder_name=experiment_folder_name), 'wb'))
